...
async function start() {
  ...
  app.get('/api/employees', async (req, res) => {
    ...
    for (const emp of employees) {
      const leads = await db.all(
        'SELECT firstname, lastname, created FROM leads WHERE employee_id=?',
        [emp.id]
      );
      const checkins = await db.all(
        'SELECT patient, notes, checkin, created FROM patient_checkins WHERE employee_id=?',
        [emp.id]
      );
      const events = await db.all(
        `SELECT e.id, e.title, e.start, e.end, e.created FROM events e JOIN event_employees ee ON ee.event_id=e.id WHERE ee.employee_id=?`,
        [emp.id]
      );
      const groups = [];
      ...
      if (events.length) {
        for (const ev of events) {
          const rows = await db.all(
            `SELECT name FROM employees e JOIN event_employees ee ON e.id=ee.employee_id WHERE ee.event_id=?`,
            [ev.id]
          );
  ...
  app.get('/api/records', async (req, res) => {
    const db = await dbPromise;
    const rows = await db.all(
      `SELECT DISTINCT r.name FROM records r JOIN formrecord f ON f.record_id=r.id AND f.form_type='quickadd' AND f.active=1`
    );
  ...
  app.post('/api/leads', async (req, res) => {
    ...
    await db.run(
      ...
      [empId, firstname, lastname, create]
    );
  ...
  app.get('/api/form/:record/:type', async (req, res) => {
    ...
    const form = await db.get(
      'SELECT id, title FROM formrecord WHERE record_id=? AND form_type=? AND active=1',
      [rec.id, type]
    );
