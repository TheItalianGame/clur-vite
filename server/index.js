...
async function start() {
  ...
  app.get('/api/employees', async (req, res) => {
    ...
    for (const emp of employees) {
      ...
      const events = await db.all(`SELECT e.id, e.title, e.start, e.end, e.created FROM events e JOIN event_employees ee ON ee.event_id=e.id WHERE ee.employee_id=?`, [emp.id]);
      const groups = [];
  ...
  app.get('/api/form/:record/:type', async (req, res) => {
    ...
    const form = await db.get('SELECT id, title FROM formrecord WHERE record_id=? AND form_type=? AND active=1', [rec.id, type]);
  ...
  app.get('/api/records', async (req, res) => {
    ...
    const rows = await db.all(`
      SELECT DISTINCT r.name FROM records r
        JOIN formrecord fr ON fr.record_id=r.id
      WHERE fr.form_type='quickadd' AND fr.active=1
    `);
